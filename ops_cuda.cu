...
thread_local CudaLinearMode g_cuda_linear_mode = CudaLinearMode::kReference;
}
thread_local CudaAttentionMode g_cuda_attention_mode = CudaAttentionMode::kReference;
CudaLinearModeGuard::CudaLinearModeGuard(CudaLinearMode mode)
    : previous_(g_cuda_linear_mode) { g_cuda_linear_mode = mode; }
CudaLinearModeGuard::~CudaLinearModeGuard() { g_cuda_linear_mode = previous_; }
CudaAttentionModeGuard::CudaAttentionModeGuard(CudaAttentionMode mode)
    : previous_(g_cuda_attention_mode) { g_cuda_attention_mode = mode; }
CudaAttentionModeGuard::~CudaAttentionModeGuard() { g_cuda_attention_mode = previous_; }
...
template<class T> __global__ void add_rms_normk(
    const T* lhs, const T* rhs, const T* weight, T* residual, T* normalized,
  ...
  for (size_t j = tid; j < hidden; j += blockDim.x) {
    ...
    normalized[row * hidden + j] = wf<T>(
        __fmul_rn(__fmul_rn(value, inv), rf(weight[j])));
  }
}
...
template<class T> __global__ void rmsk(const T* x, const T* w, T* o,
...
__global__ void gqadk_batched_variable_flash(const __half* q,
  ...
  const size_t index = blockIdx.x, row = index / qh, qhead = index % qh, dim = threadIdx.x;
  if (row >= batch || dim >= d) return;
  extern __shared__ __half smem[];
  __half* query = smem; __half* key_tile = query + d; __half* value_tile = key_tile + kTile * d;
  float* scores = reinterpret_cast<float*>(value_tile + kTile * d);
  __shared__ float partial[4];
  const size_t kvhead = qhead / (qh / kh), length = static_cast<size_t>(lengths[row]);
  const __half* qrow = q + (row * qh + qhead) * d;
  const __half* kbase = keys[row]; const __half* vbase = values[row];
  query[dim] = qrow[dim]; __syncthreads();
  float maximum = -CUDART_INF_F; double denominator = 0.0, result = 0.0;
  for (size_t begin = 0; begin < length; begin += kTile) {
    const size_t count = min(kTile, length - begin);
    for (size_t i = dim; i < count * d; i += d) {
      const size_t token = begin + i / d;
      key_tile[i] = kbase[(token * kh + kvhead) * d + i % d];
      value_tile[i] = vbase[(token * kh + kvhead) * d + i % d];
    }
    __syncthreads();
    for (size_t key = 0; key < count; ++key) {
      float part = 0.0f;
      for (size_t e = dim; e < d; e += d)
        part += __half2float(query[e]) * __half2float(key_tile[key * d + e]);
      for (int delta = 16; delta > 0; delta >>= 1) part += __shfl_down_sync(0xffffffff, part, delta);
      if ((dim % 32) == 0) partial[dim / 32] = part;
      __syncthreads();
      if (dim == 0) scores[key] = (partial[0] + partial[1] + partial[2] + partial[3]) * scale;
      __syncthreads();
    }
    if (dim == 0) { float tile_max = -CUDART_INF_F; for (size_t key = 0; key < count; ++key) tile_max = fmaxf(tile_max, scores[key]); scores[32] = tile_max; }
    __syncthreads();
    ...
    for (size_t key = 0; key < count; ++key) {
      ...
      result += double(weight) * __half2float(value_tile[key * d + dim]);
      denominator += double(weight);
    }
    maximum = next_max;
    __syncthreads();
  }
  o[(row * qh + qhead) * d + dim] = __float2half(result / denominator);
}
...
static Blas& linear_blas(){static Blas b;return b;}
class LtBlas{public:cublasLtHandle_t h{};LtBlas(){CUDA_CHECK(cublasLtCreate(&h));}~LtBlas(){if(cublasLtDestroy(h)!=CUBLAS_STATUS_SUCCESS)std::terminate();}};
static LtBlas& linear_lt_blas(){static LtBlas b;return b;}
struct LtAlgoCache { std::mutex mutex; std::unordered_map<uint64_t,cublasLtMatmulAlgo_t> algorithms; };
static LtAlgoCache& linear_lt_algo_cache(){static LtAlgoCache c;return c;}
static constexpr size_t kLinearLtWorkspaceBytes = 64ull * 1024ull * 1024ull;
static DeviceBuffer& linear_lt_workspace(){static DeviceBuffer workspace(kLinearLtWorkspaceBytes);return workspace;}
static void linear_f16_fast_gemm(const Tensor& x,const Tensor& w,Tensor& o,size_t tokens,size_t in,size_t out){
  auto& lt=linear_lt_blas(); cublasLtMatmulDesc_t op{}; cublasLtMatrixLayout_t ad{},bd{},cd{},dd{}; cublasLtMatmulPreference_t pref{};
  CUDA_CHECK(cublasLtMatmulDescCreate(&op,CUBLAS_COMPUTE_32F,CUDA_R_32F));
  cublasOperation_t ta=CUBLAS_OP_T,tb=CUBLAS_OP_N;
  CUDA_CHECK(cublasLtMatmulDescSetAttribute(op,CUBLASLT_MATMUL_DESC_TRANSA,&ta,sizeof(ta)));
  CUDA_CHECK(cublasLtMatmulDescSetAttribute(op,CUBLASLT_MATMUL_DESC_TRANSB,&tb,sizeof(tb)));
  CUDA_CHECK(cublasLtMatrixLayoutCreate(&ad,CUDA_R_16F,in,out,in)); CUDA_CHECK(cublasLtMatrixLayoutCreate(&bd,CUDA_R_16F,in,tokens,in));
  CUDA_CHECK(cublasLtMatrixLayoutCreate(&cd,CUDA_R_16F,out,tokens,out)); CUDA_CHECK(cublasLtMatrixLayoutCreate(&dd,CUDA_R_16F,out,tokens,out));
  const size_t workspace_bytes=kLinearLtWorkspaceBytes; cublasLtMatmulAlgo_t algo{};
  const uint64_t key=(uint64_t(tokens)*1000003ull+uint64_t(in))*1000003ull+uint64_t(out);
  bool cached=false;
  { auto& cache=linear_lt_algo_cache(); std::lock_guard<std::mutex> lock(cache.mutex); auto it=cache.algorithms.find(key); if(it!=cache.algorithms.end()){algo=it->second;cached=true;} }
  if(!cached){
    CUDA_CHECK(cublasLtMatmulPreferenceCreate(&pref));
    CUDA_CHECK(cublasLtMatmulPreferenceSetAttribute(pref,CUBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES,&workspace_bytes,sizeof(workspace_bytes)));
    cublasLtMatmulHeuristicResult_t result{}; int count=0;
    CUDA_CHECK(cublasLtMatmulAlgoGetHeuristic(lt.h,op,ad,bd,cd,dd,pref,1,&result,&count));
    if(count==0||result.state!=CUBLAS_STATUS_SUCCESS) throw std::runtime_error("cuda_linear: cuBLASLt found no compatible FP16 algorithm");
    algo=result.algo; auto& cache=linear_lt_algo_cache(); std::lock_guard<std::mutex> lock(cache.mutex); cache.algorithms.emplace(key,algo);
  }
  const float alpha=1.0f,beta=0.0f;
  CUDA_CHECK(cublasLtMatmul(lt.h,op,&alpha,w.data(),ad,x.data(),bd,&beta,o.data(),cd,o.data(),dd,&algo,linear_lt_workspace().get(),workspace_bytes,0));
  if(pref) CUDA_CHECK(cublasLtMatmulPreferenceDestroy(pref)); CUDA_CHECK(cublasLtMatrixLayoutDestroy(ad)); CUDA_CHECK(cublasLtMatrixLayoutDestroy(bd)); CUDA_CHECK(cublasLtMatrixLayoutDestroy(cd)); CUDA_CHECK(cublasLtMatrixLayoutDestroy(dd)); CUDA_CHECK(cublasLtMatmulDescDestroy(op));
}
...
void cuda_add_rms_norm_out(const Tensor& lhs, const Tensor& rhs,
                           ...
                           cudaStream_t stream) {
  req(lhs, "cuda_add_rms_norm");
  req(rhs, "cuda_add_rms_norm");
  req(weight, "cuda_add_rms_norm");
  same_dtype(lhs, rhs, "cuda_add_rms_norm");
  same_dtype(lhs, weight, "cuda_add_rms_norm");
  rank(weight, 1, "cuda_add_rms_norm");
  if (lhs.shape() != rhs.shape() || lhs.shape().empty() ||
      lhs.shape().back() != weight.shape()[0] || eps <= 0.0f)
    throw std::invalid_argument("cuda_add_rms_norm: incompatible shapes/epsilon");
  output_req(residual, lhs.shape(), lhs.dtype(), "cuda_add_rms_norm");
  output_req(normalized, lhs.shape(), lhs.dtype(), "cuda_add_rms_norm");
  const size_t rows = lhs.numel() / static_cast<size_t>(lhs.shape().back());
  const size_t hidden = static_cast<size_t>(lhs.shape().back());
  if (lhs.dtype() == DType::F32)
    add_rms_normk<<<static_cast<unsigned int>(rows), 128, 0, stream>>>(
        static_cast<const float*>(lhs.data()), static_cast<const float*>(rhs.data()),
        static_cast<const float*>(weight.data()), static_cast<float*>(residual.data()),
        static_cast<float*>(normalized.data()), rows, hidden, eps);
  else
    add_rms_normk<<<static_cast<unsigned int>(rows), 128, 0, stream>>>(
        static_cast<const __half*>(lhs.data()), static_cast<const __half*>(rhs.data()),
        static_cast<const __half*>(weight.data()), static_cast<__half*>(residual.data()),
        static_cast<__half*>(normalized.data()), rows, hidden, eps);
  CUDA_KERNEL_CHECK();
}
...
void cuda_swiglu_out(const Tensor&a,const Tensor&b,Tensor&o,cudaStream_t stream){req(a,"cuda_swiglu");req(b,"cuda_swiglu");same_dtype(a,b,"cuda_swiglu");if(a.shape()!=b.shape())throw std::invalid_argument("cuda_swiglu: shapes must match");output_req(o,a.shape(),a.dtype(),"cuda_swiglu");size_t n=a.numel();if(a.dtype()==DType::F32)siluk<<<(n+255)/256,256,0,stream>>>((float*)a.data(),(float*)b.data(),(float*)o.data(),n);else siluk<<<(n+255)/256,256,0,stream>>>((__half*)a.data(),(__half*)b.data(),(__half*)o.data(),n);CUDA_KERNEL_CHECK();}
...
void cuda_embedding_lookup_device_ids_out(const Tensor&w,const Tensor&ids,Tensor&out){
  req(w,"cuda_embedding_lookup_device_ids"); rank(w,2,"cuda_embedding_lookup_device_ids");
  if(ids.device()!=DeviceType::CUDA||ids.dtype()!=DType::F32||!ids.is_contiguous()||ids.shape().size()!=1||ids.shape()[0]<=0)
    throw std::invalid_argument("cuda_embedding_lookup_device_ids: ids must be CUDA/F32/contiguous rank-1");
  const int64_t vocab=w.shape()[0], hidden=w.shape()[1];
  output_req(out,{ids.shape()[0],hidden},w.dtype(),"cuda_embedding_lookup_device_ids");
  const size_t n=static_cast<size_t>(ids.shape()[0])*static_cast<size_t>(hidden);
  if(w.dtype()==DType::F32) embeddingk<<<(n+255)/256,256>>>((const float*)w.data(),(const int32_t*)ids.data(),(float*)out.data(),ids.shape()[0],hidden,vocab);
  else embeddingk<<<(n+255)/256,256>>>((const __half*)w.data(),(const int32_t*)ids.data(),(__half*)out.data(),ids.shape()[0],hidden,vocab);
  CUDA_KERNEL_CHECK();
}
...
void cuda_gqa_decode_attention_batched_variable_lengths_out(
  ...
  if (g_cuda_attention_mode == CudaAttentionMode::kFlashOnline) {
    const unsigned int shared_bytes = static_cast<unsigned int>(
        (128 + 2 * 32 * 128) * sizeof(__half) + 37 * sizeof(float));
    gqadk_batched_variable_flash<<<static_cast<unsigned int>(batch * 16), 128, shared_bytes>>>(
        static_cast<const __half*>(q.data()), static_cast<const __half* const*>(workspace.keys.get()),
        static_cast<const __half* const*>(workspace.values.get()), static_cast<const int32_t*>(lengths.data()),
        static_cast<__half*>(output.data()), batch, 16, 8, 128, 1.0f / sqrtf(128.0f));
  } else {
    gqadk_batched_variable<<<(n + 255) / 256, 256>>>(
        static_cast<const __half*>(q.data()), static_cast<const __half* const*>(workspace.keys.get()),
        static_cast<const __half* const*>(workspace.values.get()), static_cast<const int32_t*>(lengths.data()),
...
void cuda_rope_token_positions_out(const Tensor& x, const Tensor& positions_device,
                                   float theta, Tensor& o, cudaStream_t stream) {
  ...
  const size_t n = static_cast<size_t>(x.shape()[0]) *
                   static_cast<size_t>(x.shape()[1]) * 64;
  rope_decode_batched_kernel<<<(n + 255) / 256, 256, 0, stream>>>(
      static_cast<const __half*>(x.data()), static_cast<__half*>(o.data()),
      static_cast<const int32_t*>(positions_device.data()),
      static_cast<size_t>(x.shape()[0]), static_cast<size_t>(x.shape()[1]),
      128, theta);
