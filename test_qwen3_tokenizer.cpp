...
int main() {
  try {
    ...
    while ((pos=json.find("{",pos)) != std::string::npos) {
      size_t end=json.find("}\n",pos); if(end==std::string::npos) end=json.find('}',pos); if(end==std::string::npos) break;
      const std::string object=json.substr(pos,end-pos+1); if(object.find("\"name\"")==std::string::npos) break;
