...
impl TokenizerWrapper {
    pub fn from_str(json: &str) -> Option<TokenizerWrapper> {
        Some(TokenizerWrapper {
            tokenizer: Tokenizer::from_str(json).ok()?.into(),
...
extern "C" fn tokenizers_new_from_str(input_cstr: *const u8, len: usize) -> *mut TokenizerWrapper {
    unsafe {
        ...
        match TokenizerWrapper::from_str(json) {
            Some(tokenizer) => return Box::into_raw(Box::new(tokenizer)),
