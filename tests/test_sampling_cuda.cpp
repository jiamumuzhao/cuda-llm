...
int main() {
  try {
    ...
    Tensor crossing_cpu = Tensor::from_f32({1,6}, {std::log(0.505f), std::log(0.495f), -20.f, -20.f, -20.f, -20.f});
    Tensor crossing = crossing_cpu.to(DeviceType::CUDA);
    SamplingConfig crossing_cfg; crossing_cfg.temperature=1.f; crossing_cfg.top_k=0; crossing_cfg.top_p=.51f; crossing_cfg.seed=77;
    bool saw_crossing = false;
    for (uint64_t offset=0; offset<64; ++offset) {
      int32_t id = sampler.sample_last_row(crossing, crossing_cfg, offset);
      if (id != 0 && id != 1) fail("top-p crossing selected a truncated token");
      if (id == 1) saw_crossing = true;
    }
    if (!saw_crossing) fail("top-p crossing token was never sampled");
    std::cout << "top-p crossing: retained tokens={0,1}, crossing token observed\n";
    Tensor crossing_f16 = f16(crossing_cpu).to(DeviceType::CUDA);
    bool saw_crossing_f16 = false;
    for (uint64_t offset=0; offset<64; ++offset) {
      int32_t id = sampler.sample_last_row(crossing_f16, crossing_cfg, offset);
      if (id != 0 && id != 1) fail("F16 top-p crossing selected a truncated token");
      if (id == 1) saw_crossing_f16 = true;
    }
    if (!saw_crossing_f16) fail("F16 top-p crossing token was never sampled");
    std::cout << "F16 sampling: crossing token observed\n";
    Tensor ties_cpu = Tensor::from_f32({1,6}, {0.f, 0.f, 0.f, -10.f, -10.f, -10.f});
    Tensor ties = ties_cpu.to(DeviceType::CUDA);
    SamplingConfig tie_cfg; tie_cfg.temperature=1.f; tie_cfg.top_k=0; tie_cfg.top_p=.30f; tie_cfg.seed=88;
    for (uint64_t offset=0; offset<64; ++offset)
      if (sampler.sample_last_row(ties, tie_cfg, offset) != 0) fail("F32 tie-order did not prefer lowest token ID");
    Tensor ties_f16 = f16(ties_cpu).to(DeviceType::CUDA);
    for (uint64_t offset=0; offset<64; ++offset)
      if (sampler.sample_last_row(ties_f16, tie_cfg, offset) != 0) fail("F16 tie-order did not prefer lowest token ID");
    std::cout << "tie-order: F32/F16 equal maxima always selected token 0\n";
