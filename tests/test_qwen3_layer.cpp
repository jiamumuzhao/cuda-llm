...
static std::map<std::string,float> config(const fs::path& p){std::ifstream in(p);if(!in)fail("config missing: "+p.string());std::map<std::string,float> r;std::string line;while(std::getline(in,line)){std::istringstream ss(line);std::string k,v;if(!(ss>>k>>v))continue;try{r[k]=std::stof(v);}catch(...){}}return r;}
...
int main(){
 ...
 fs::path dir="artifacts/phase1/qwen3_layer0";if(!fs::exists(dir/"manifest.txt") && fs::exists("../artifacts/phase1/qwen3_layer0/manifest.txt"))dir="../artifacts/phase1/qwen3_layer0";if(!fs::exists(dir/"manifest.txt"))fail("fixture manifest missing: "+(dir/"manifest.txt").string());
