...
int main() {
  try {
    ...
    for (size_t batch_size : {size_t(1), size_t(2), size_t(4)}) {
      ...
      batched_storage.reserve(batch_size);
      for (size_t b = 0; b < batch_size; ++b) {
    ...
    base_caches.reserve(4); changed_caches.reserve(4);
    ...
    expect_throw("mismatched cache count", [&] { std::vector<Qwen3KvCache> c; c.emplace_back(32); c.emplace_back(32); std::vector<Qwen3KvCache*> p{&c[0]}; model.prefill_logits_batch_with_caches(good_prompts, p); });
    expect_throw("unequal sequence lengths", [&] { std::vector<Qwen3KvCache> c; c.emplace_back(32); c.emplace_back(32); std::vector<Qwen3KvCache*> p{&c[0], &c[1]}; model.prefill_logits_batch_with_caches({good_prompts[0], prompt(1, 3, 7)}, p); });
    expect_throw("token out of range", [&] { auto bad=good_prompts; bad[0][0]=151936; std::vector<Qwen3KvCache> c; c.emplace_back(32); c.emplace_back(32); std::vector<Qwen3KvCache*> p{&c[0],&c[1]}; model.prefill_logits_batch_with_caches(bad,p); });
    expect_throw("capacity too small", [&] { std::vector<Qwen3KvCache> c; c.emplace_back(2); c.emplace_back(2); std::vector<Qwen3KvCache*> p{&c[0],&c[1]}; model.prefill_logits_batch_with_caches(good_prompts,p); });
    ...
    Tensor health_output = cuda_add(health_device, health_device);
    Tensor health_result = health_output.to(DeviceType::CPU);
    for (size_t i = 0; i < 4; ++i) if (health_result.get_f32(i) != 2.0f * health_host.get_f32(i)) fail("CUDA health check mismatch");
    CUDA_CHECK(cudaDeviceSynchronize());
    std::cout << "post-batch CUDA health check passed\n";
