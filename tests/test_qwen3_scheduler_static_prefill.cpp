...
int main() {
  try {
    ...
    if (scheduler.prefill_waiting_static_batch(4) != 2) fail("static batch did not select supported equal-length batch=2");
    if (scheduler.waiting_count()!=2 || scheduler.state(3)!=RequestState::Waiting || scheduler.decode_count()!=2) fail("FIFO/static grouping state mismatch");
    std::cout << "static_prefill selected=2 anchor_seq_len=4 max_batch_size=4 unselected_fifo_preserved=passed\n";
    if (scheduler.prefill_waiting_static_batch(4) != 1 || scheduler.decode_count()!=3) fail("second static batch did not select seq_len=8 request");
    if (scheduler.prefill_waiting_static_batch(4) != 1 || scheduler.decode_count()!=4) fail("third static batch did not select remaining seq_len=4 request");
    ...
    if (stops.prefill_waiting_static_batch(4)!=2 || stops.prefill_waiting_static_batch(4)!=1) fail("stop cases were not statically grouped");
    while(stops.step()){}
    ...
    std::cout << "post-static-scheduler CUDA health check passed\n";
