...
__global__ void paged_gqa_decode_kernel(
  ...
  for (std::size_t token = 0; token < kv_length; ++token) {
    ...
    for (std::size_t pair = dim; pair * 2 + 1 < head_dim;
      ...
      partial += q_value.x * k_value.x;
      partial += q_value.y * k_value.y;
    }
    if (2 * dim + 1 == head_dim)
    ...
    if (dim == 0) {
      ...
      score_workspace[0] = total * scale;
    }
    __syncthreads();

    const float score = score_workspace[0];
    ...
    result = result * old_scale + weight * __half2float(value[dim]);
...
__global__ void paged_gqa_decode_batch_kernel(
  ...
  for (std::size_t token = 0; token < kv_length; ++token) {
    ...
    __syncthreads();
    if (dim == 0) {
      float score = score_workspace[0];
      for (std::size_t w = 1; w < (head_dim + 31) / 32; ++w)
        score += score_workspace[w];
      score_workspace[0] = score * scale;
    }
    __syncthreads();
    const float score = score_workspace[0];
    const float next_max = fmaxf(running_max, score);
    const float old_scale = expf(running_max - next_max);
    const float weight = expf(score - next_max);
    const std::size_t value_block =
        static_cast<std::size_t>(table[token / block_size]);
    const std::size_t value_offset = token % block_size;
    const std::size_t value_layer_block = (layer * total_blocks + value_block) * 2;
    const __half* value = storage + (value_layer_block + 1) * plane_elements +
                          value_offset * token_stride + kv_head * head_dim;
    denominator = denominator * old_scale + weight;
    result = result * old_scale + weight * __half2float(value[dim]);
    running_max = next_max;
  }
