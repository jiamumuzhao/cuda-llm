...
Qwen3LayerTrace run_trace(
  ...
  Tensor q = r.q_linear.reshape({seq, 16, 128});
  Tensor k = r.k_linear.reshape({seq, 8, 128});
  Tensor v = r.v_linear.reshape({seq, 8, 128});
  ...
  r.attention_output = attention_heads.reshape({seq, 2048});
...
Tensor qwen3_decoder_layer_cuda_decode_fp16(
  ...
  require_tensor(k_cache_layer, {k_cache_layer.shape().empty() ? 0 : k_cache_layer.shape()[0], 8, 128}, "k_cache", DType::F16, function);
  ...
  const size_t row_bytes = 8 * 128 * dtype_size(DType::F16);
  ...
  Tensor attention = cuda_gqa_decode_attention(q_rope, k_cache_layer, v_cache_layer,
                                               previous_cache_length + 1).reshape({1, 2048});
...
Tensor qwen3_decoder_layer_cuda_decode_fp16_paged_batch(
  ...
  require_tensor(hidden_bh, {static_cast<int64_t>(batch), 1024}, "hidden", DType::F16, function);
  ...
  Tensor q_linear = cuda_linear(norm, w.q_proj).reshape({static_cast<int64_t>(batch),16,128});
  Tensor k_linear = cuda_linear(norm, w.k_proj).reshape({static_cast<int64_t>(batch),8,128});
  Tensor v_linear = cuda_linear(norm, w.v_proj).reshape({static_cast<int64_t>(batch),8,128});
  Tensor q_norm = cuda_rms_norm(q_linear, w.q_norm, eps);
  Tensor k_norm = cuda_rms_norm(k_linear, w.k_norm, eps);
  ...
  Tensor attention_heads(DType::F16,
                         {static_cast<int64_t>(batch), 16, 128},
                         DeviceType::CUDA);
  for (size_t b = 0; b < batch; ++b) {
    if (g_paged_batch_fault_row == b && g_paged_batch_fault_layer == layer_index)
      throw std::runtime_error("paged batch injected fault");
    Tensor k_row = k_rope.slice_first_dim(b, 1).reshape({8, 128});
    Tensor v_row = v_linear.slice_first_dim(b, 1).reshape({8, 128});
...
void qwen3_decoder_layer_cuda_decode_fp16_paged_batch_into(
  ...
  if (batch != 1 && batch != 2 && batch != 4) invalid(function, "batch must be 1, 2, or 4");
  require_tensor(hidden_bh, {static_cast<int64_t>(batch), 1024}, "hidden", DType::F16, function);
  require_tensor(output_bh, {static_cast<int64_t>(batch), 1024}, "output", DType::F16, function);
  ...
  Tensor q = q_linear.reshape({static_cast<int64_t>(batch), 16, 128});
  Tensor k = k_linear.reshape({static_cast<int64_t>(batch), 8, 128});
  Tensor v = v_linear.reshape({static_cast<int64_t>(batch), 8, 128});
  ...
  cuda_rope_batched_positions_out(q, positions_cuda, theta, q);
  cuda_rope_batched_positions_out(k, positions_cuda, theta, k);
  for (size_t b = 0; b < batch; ++b) {
    if (g_paged_batch_fault_row == b && g_paged_batch_fault_layer == layer_index)
      throw std::runtime_error("paged batch injected fault");
    Tensor k_row = k.slice_first_dim(b, 1).reshape({8, 128});
    Tensor v_row = v.slice_first_dim(b, 1).reshape({8, 128});
    caches[b]->append_layer_kv(layer_index, k_row, v_row);
  }
...
static Qwen3BatchLayerTrace qwen3_decoder_layer_cuda_trace_fp16_batch_impl(
  ...
  if (hidden_bsh.shape().size() != 3)
    invalid(function, "hidden_bsh must have shape [B,S,1024]");
  const int64_t batch = hidden_bsh.shape()[0], seq = hidden_bsh.shape()[1];
  require_tensor(hidden_bsh, {batch, seq, 1024}, "hidden_bsh", DType::F16, function);
  ...
  Tensor q_linear = cuda_linear(norm, w.q_proj).reshape({batch,seq,16,128});
  Tensor k_linear = cuda_linear(norm, w.k_proj).reshape({batch,seq,8,128});
  Tensor v_linear = cuda_linear(norm, w.v_proj).reshape({batch,seq,8,128});
  Tensor q_norm = cuda_rms_norm(q_linear, w.q_norm, eps);
  Tensor k_norm = cuda_rms_norm(k_linear, w.k_norm, eps);
  ...
  Tensor attention = (valid_lengths_cuda
      ...
      : cuda_gqa_attention_batched(q_rope, k_rope, v_linear)).reshape({batch,seq,2048});
  Tensor o_proj = cuda_linear(attention, w.o_proj);
  Tensor attention_residual = cuda_add(residual, o_proj);
  Tensor post_norm = cuda_rms_norm(attention_residual, w.post_attention_norm, eps);
  Tensor gate = cuda_linear(post_norm, w.gate_proj);
...
Tensor qwen3_decoder_layer_cuda_decode_fp16_batch(
  ...
  require_tensor(hidden_bh, {batch, 1024}, "hidden_bh", DType::F16, function);
  ...
  for (size_t b = 0; b < caches.size(); ++b) {
    ...
    if (caches[b]->length() != previous_length || previous_length == 0 ||
        previous_length >= caches[b]->capacity() || caches[b]->num_kv_heads() != 8 ||
  ...
  Tensor residual = hidden_bh;
  Tensor norm = cuda_rms_norm(hidden_bh, w.input_norm, eps);
  Tensor q = cuda_linear(norm, w.q_proj).reshape({batch, 16, 128});
  Tensor k = cuda_linear(norm, w.k_proj).reshape({batch, 8, 128});
  Tensor v = cuda_linear(norm, w.v_proj).reshape({batch, 8, 128});
  Tensor q_norm = cuda_rms_norm(q, w.q_norm, eps);
  Tensor k_norm = cuda_rms_norm(k, w.k_norm, eps);
  Tensor q_rope = cuda_rope_batched(q_norm.reshape({batch,1,16,128}), {shared_position}, theta)
                     .reshape({batch,16,128});
  Tensor k_rope = cuda_rope_batched(k_norm.reshape({batch,1,8,128}), {shared_position}, theta)
                     .reshape({batch,8,128});
  for (size_t b = 0; b < caches.size(); ++b)
...
void qwen3_decoder_layer_cuda_decode_fp16_batch_into(
  ...
  require_tensor(hidden_bh, {static_cast<int64_t>(batch), 1024}, "hidden", DType::F16, function);
  require_tensor(output, {static_cast<int64_t>(batch), 1024}, "output", DType::F16, function);
  ...
  cuda_rms_norm_out(q, w.q_norm, eps, q);
  cuda_rms_norm_out(k, w.k_norm, eps, k);
  Tensor q_rope_view = q.reshape({static_cast<int64_t>(batch),1,16,128});
  Tensor k_rope_view = k.reshape({static_cast<int64_t>(batch),1,8,128});
