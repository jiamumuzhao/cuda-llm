...
Qwen3LayerTrace qwen3_decoder_layer_trace(const Tensor& hidden_states, const std::vector<int32_t>& positions,
  ...
  auto attention_heads = gqa_attention(r.q_rope, r.k_rope, v);
  r.attention_output = attention_heads.reshape({hidden_states.shape()[0],2048});
  r.o_proj_output = linear(r.attention_output, w.o_proj);
